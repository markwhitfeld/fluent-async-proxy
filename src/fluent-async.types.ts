...
type WrappedValue<T> = T extends Promise<infer V> ? FluentPromise<V> : T extends AnyFunc ? WrappedFunc<T> : T extends object ? Wrapped<T> : T;
export type WrappedFunc<T extends AnyFunc> = T extends (
  ...args: infer TArgs
) => infer TReturn ? TReturn extends Promise<infer V> ? (...args: TArgs) => FluentPromise<V> : (...args: TArgs) => Wrapped<TReturn> : never;
...
export type PromisedFunc<T extends AnyFunc> = (T extends (
  ...args: infer TArgs
) => infer TReturn ? (...args: TArgs) => FluentPromise<Promised<TReturn>> : never) &
  Promise<Wrapped<T>>;
